...
FrameBufferTest::FrameBufferTest(SDL_Renderer *renderer): FrameBuffer()
{
    FrameBufferStatus status = init(renderer, m_pixel_buffer,
                                                   FRAME_BUFFER_TEST_WIDTH,
                                                   FRAME_BUFFER_TEST_HEIGHT,
                                                   SDL_PIXELFORMAT_RGB332);

    //memset(frame_buffer->screen_ram, 0x40, FRAME_BUFFER_TEST_RAM_SCREEN_SIZE);
    memcpy(m_screen_ram, S_test_data, FRAME_BUFFER_TEST_RAM_SCREEN_SIZE);
...
FrameBufferStatus FrameBufferTest::update()
{
    ...
    for (pixel_row = 0; pixel_row < m_pixel_buffer_height; pixel_row++)
    {
        for (pixel_col = 0; pixel_col < m_pixel_buffer_width; pixel_col++)
        {
            ...
            uint8_t screen_char = m_screen_ram[(screen_row * 32) + screen_col];
            ...
            if (chr_bitmap_row & (0x80 >> screen_col_bit))
                m_pixel_buffer[(pixel_row * m_pixel_buffer_width) + pixel_col] = 0xff;
            else
                m_pixel_buffer[(pixel_row * m_pixel_buffer_width) + pixel_col] = 0x00;

        }

    }

    status = FrameBuffer::update();
